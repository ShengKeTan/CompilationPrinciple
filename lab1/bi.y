...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef  Specifier ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt StmList DefList Def DecList Dec Exp Args StructSpecifire
...
%token <type_struct> STURCT

%token LP RP LC RC SEMI COMMA   //用bison对该文件编译时，带参数-d，生成的exp.tab.h中给这些单词进行编码，可在lex.l中包含parser.tab.h使用这些单词种类码
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN ADDADD SUBSUB
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
	  ...
	  | ADDADD Exp    {$$=mknode(ADDADD,$2,NULL,NULL,yylineno);strcpy($$->type_id,"ADDADD");}
	  | Exp ADDADD	  {$$=mknode(ADDADD,$1,NULL,NULL,yylineno);strcpy($$->type_id,"ADDADD");}
	  | SUBSUB Exp    {$$=mknode(SUBSUB,$2,NULL,NULL,yylineno);strcpy($$->type_id,"SUBSUB");}
	  | Exp SUBSUB	  {$$=mknode(SUBSUB,$1,NULL,NULL,yylineno);strcpy($$->type_id,"SUBSUB");}
      | LP Exp RP     {$$=$2;}
